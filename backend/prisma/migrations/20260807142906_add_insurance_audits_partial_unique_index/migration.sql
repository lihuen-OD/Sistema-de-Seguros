-- Auditoría de Seguros — índice único PARCIAL sobre (assetId, auditPeriod),
-- excluyendo auditorías REJECTED/NEEDS_CORRECTION, mismo patrón que
-- asset_audits y fire_extinguisher_audits (ver migración
...
-- modelo InsuranceAudit).
--
-- El nombre del índice conserva el substring literal "auditPeriod" a
-- propósito: handleDuplicateAudit() en insurance-audits.service.ts detecta
-- el 409 con target.includes('auditPeriod') sobre e.meta.target, y Prisma no
...
-- insurance_audits, para confirmar que no elimina este índice.

CREATE UNIQUE INDEX "insurance_audits_assetId_auditPeriod_active_key"
  ON "insurance_audits" ("assetId", "auditPeriod")
