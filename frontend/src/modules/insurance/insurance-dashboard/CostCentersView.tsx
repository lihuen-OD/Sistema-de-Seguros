...
import { Wallet, Gauge, Layers3, WalletCards } from 'lucide-react'
...
import type { CostCenterInsuranceSummary } from '../../../shared/utils/insuranceDashboardCalc'
import type { TableColumn } from '../../../shared/types'
import { ChartMeasureToggle, type ChartMeasure } from './ChartMeasureToggle'

interface CostCentersViewProps {
  summaries: CostCenterInsuranceSummary[]
}
...
function barColor(kind: CostCenterInsuranceSummary['kind'], index: number): string {
...
// Mismo componente que ProductiveUnitsView.tsx/FixedAssetsView.tsx, aplicado
// a Centro de Costo — ver computeCostCenterSummaries: acá el gasto de un
// activo repartido entre varios centros de costo llega YA prorrateado por
// porcentaje, así que no existe un bucket "compartido" como en las otras dos
// vistas (no hay ambigüedad que resolver, el % ya define el reparto).
export function CostCentersView({ summaries }: CostCentersViewProps) {
  ...
  const metrics = useMemo(() => {
    ...
    const costCenters = summaries.filter((item) => item.kind === 'unit')
    const totalAssets = summaries.reduce((total, item) => total + item.assetCount, 0)
    const leadingUnit = costCenters.find((item) => item.facturado12mUsd > 0) ?? null
    const unassigned = summaries.find((item) => item.kind === 'unassigned') ?? null
    return {
      totalSpend,
      actualUnitsCount: costCenters.length,
      totalAssets,
  ...
  const columns: TableColumn<CostCenterInsuranceSummary>[] = [
    {
      id: 'label',
      key: 'label',
      ...
      render: (_, row) => (
        <div className="min-w-0">
          ...
          {row.kind === 'unassigned' && (
            <span className="ml-2 inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-slate-100 text-slate-500">
              Dato pendiente
            </span>
          )}
        </div>
    ...
    <div className="space-y-5">
      <MetricGrid cols={4}>
        <KpiCard
          ...
          description="Documentos de los últimos 12 meses, prorrateados por % de imputación"
          icon={WalletCards}
          variant="info"
        />
        <KpiCard
        ...
        <KpiCard
          ...
          description={
            metrics.unassigned && metrics.unassigned.facturado12mUsd > 0
              ? `${percentage(metrics.unassigned.sharePct)} sin centro de costo asignado`
              : 'Todo el gasto quedó asignado a un centro de costo'
          }
          icon={Layers3}
          variant={metrics.unassigned && metrics.unassigned.facturado12mUsd > 0 ? 'warning' : 'success'}
        />
      </MetricGrid>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <ChartCard
          title={
            chartMeasure === 'amount'
              ? 'Gasto de seguros por centro de costo'
              : 'Cantidad de pólizas por centro de costo'
          }
          subtitle={
            chartMeasure === 'amount'
              ? 'Facturado en los últimos 12 meses · USD · prorrateado por % de imputación de cada activo'
            ...
            <div className="h-full flex items-center justify-center">
              <p className="text-sm text-slate-400">
                {chartMeasure === 'amount'
                  ...
                  : 'No hay pólizas vinculadas a centros de costo.'}
              </p>
            </div>
          ) : (
        ...
        <SectionCard
          ...
          subtitle="Cómo interpretar las diferencias entre centros de costo"
        >
          <div className="space-y-5">
            <div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <p className="mt-1 text-sm text-slate-700 leading-relaxed">
                “Gasto / activo” permite comparar centros de costo de distinto tamaño. “Vs. promedio” muestra cuánto se aleja cada uno del promedio general.
              </p>
            </div>

            <div className="border-t border-slate-100 pt-4">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Activos con reparto</p>
              <p className="mt-1 text-sm text-slate-700 leading-relaxed">
                Un activo imputado a más de un centro de costo aporta su gasto a cada uno según el % de esa imputación — puede aparecer contado en más de una fila de la tabla de abajo.
              </p>
            </div>
          </div>
        </SectionCard>
      ...
      >
        <DataTable
          tableKey="insurance-dashboard-cost-centers"
          columns={columns}
          data={summaries}
          rowKey="id"
          minWidth={980}
          emptyTitle="Sin datos"
          emptyDescription="No hay centros de costo para comparar."
        />
      </SectionCard>
    </div>
  )
