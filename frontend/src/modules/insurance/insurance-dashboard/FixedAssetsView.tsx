...
import { Warehouse, Gauge, Layers3, WalletCards } from 'lucide-react'
...
import {
  SHARED_FIXED_ASSET_LABEL,
  type FixedAssetInsuranceSummary,
...
interface FixedAssetsViewProps {
  summaries: FixedAssetInsuranceSummary[]
}
...
function barColor(kind: FixedAssetInsuranceSummary['kind'], index: number): string {
...
// Mismo componente que ProductiveUnitsView.tsx, aplicado a Bien de Uso en vez
// de Unidad Productiva — ver computeFixedAssetSummaries para el criterio de
// agrupamiento (idéntico al de unidades productivas: un activo tiene como
// mucho un Bien de Uso, sin necesidad de prorratear nada).
export function FixedAssetsView({ summaries }: FixedAssetsViewProps) {
  ...
  const metrics = useMemo(() => {
    ...
    const fixedAssets = summaries.filter((item) => item.kind === 'unit')
    ...
    const leadingUnit = fixedAssets.find((item) => item.facturado12mUsd > 0) ?? null
    const shared = summaries.find((item) => item.label === SHARED_FIXED_ASSET_LABEL) ?? null
    return {
      totalSpend,
      actualUnitsCount: fixedAssets.length,
      totalAssets,
  ...
  const columns: TableColumn<FixedAssetInsuranceSummary>[] = [
    {
      id: 'label',
      key: 'label',
      label: 'Bien de Uso',
    ...
    <div className="space-y-5">
        ...
        <KpiCard
          ...
          description={
            metrics.shared && metrics.shared.facturado12mUsd > 0
              ? `${percentage(metrics.shared.sharePct)} requiere distribución entre Bienes de Uso`
              : 'Todo el gasto quedó asignado a un Bien de Uso'
          }
          icon={Layers3}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <ChartCard
          title={
            chartMeasure === 'amount'
              ? 'Gasto de seguros por Bien de Uso'
              : 'Cantidad de pólizas por Bien de Uso'
          }
          subtitle={
            chartMeasure === 'amount'
            ...
            <div className="h-full flex items-center justify-center">
              <p className="text-sm text-slate-400">
                {chartMeasure === 'amount'
                  ...
                  : 'No hay pólizas vinculadas a Bienes de Uso.'}
              </p>
            </div>
          ) : (
        ...
        <SectionCard
          ...
          subtitle="Cómo interpretar las diferencias entre Bienes de Uso"
        >
          <div className="space-y-5">
            <div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <p className="mt-1 text-sm text-slate-700 leading-relaxed">
                “Gasto / activo” permite comparar Bienes de Uso de distinto tamaño. “Vs. promedio” muestra cuánto se aleja cada uno del promedio general.
              </p>
            </div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <p className="mt-1 text-sm text-slate-700 leading-relaxed">
                {metrics.shared && metrics.shared.facturado12mUsd > 0
                  ? <>Hay {formatCurrencyCompact(metrics.shared.facturado12mUsd, 'USD')} sin prorratear porque cubre activos de Bienes de Uso diferentes.</>
                  : 'No hay gasto compartido pendiente de distribución entre Bienes de Uso.'}
              </p>
            </div>
          </div>
        </SectionCard>
      ...
      >
        <DataTable
          tableKey="insurance-dashboard-fixed-assets"
          columns={columns}
          data={summaries}
          rowKey="id"
          minWidth={980}
          emptyTitle="Sin datos"
          emptyDescription="No hay Bienes de Uso para comparar."
        />
      </SectionCard>
    </div>
  )
